...
5.2. **Regarding the "Stretch Goal : Formal Verification"**
   1. I intended working my way into this part. After installing OSS Cad Suite, I quickly figured out I couldn't use my designed VHDL timer as the only command that was able to bind VHDL and SV (SystemVerilog) aka *verific* wasn't able in the free open source version of OSS Cad Suite.
   2. Hence, I decided to manually translate my VHDL code in SV (this code is provided in the *src* folder). Assertions were written in SV so I wanted to only use SV files (including assertions directly in my code). I replicated my VHDL timer code in SV, checked its behaviour through simulations to assure that it behave like my VHDL timer (via Vivado).
